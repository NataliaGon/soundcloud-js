...
const songsIconBtns = document.getElementsByClassName('icon-heart-song-explore');
...
function createSongsList(songs) {
    ...
    for (song of songs) {
        ...
        <span class="icon-heart-song-explore icon-heart" data-id="' + song.id + '"></span>\
...
function createPlaylistsList(playlists) {
    ...
    for (each of playlists) {
       ...
       <div class="container-for-songs-in-playlist">';
        ...
        <span class="icon-clock"></span>\
        <span class="song-time"></span>\
        <span class="icon-heart-song-playlist icon-heart" data-mark="'+ trek.songIdInPlaylist + '" data-id="' + trek.id + '"></span>\
        <div class="dropdown dropdown-in-playlist" data-mark="'+ trek.songIdInPlaylist + '" data-id="' + trek.id + '">\
            <h3>Add to playlist</h3>\
            <ul class="container-for-playlists-title" data-mark="'+ trek.songIdInPlaylist + '" data-id="' + trek.id + '" ></ul>\
            <a href="#">Create playlist</a>\
            <hr/>\
        </div>\
    </div>';
    ...
    savePlaylists();
    titleForNav();
    const dropDownSongPlaylist = document.getElementsByClassName('icon-heart-song-playlist');
...
function songOpenDrop() {
    ...
    let ulTitleOfPlaylists = document.querySelector(`ul[data-id="${event.target.dataset.id}"]`);
    let songId = event.target.dataset.id;
    let inputsValue = [];
    let titlesOfPlaylists = '';
    const allInputs = document.getElementsByClassName('input-playlist-title');
...
function songOpenDropInPlaylist(dropdownSong) {
    ...
    if (dropdownSong.classList.contains('display')) {
        ...
        const ulTitleOfPlaylists = document.querySelector(`ul[data-mark="${event.target.dataset.mark}"]`);
